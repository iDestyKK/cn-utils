 ...
 * To Lower
 *
 * Reads a file and pipes to stdout the same file with all ASCII characters set to lowercase.
...
main(int argc, char** argv) {
    if (argc != 2) {
        fprintf(stderr, (argc == 1) ? "USAGE: lower filename\n" : "ERROR: Expected 1 argument, received %d.\n", argc - 1);
        exit(1);
    }
    ...
    cdml_transform(lines, sizeof(char**), ln, to_lower);
